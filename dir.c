			   ...
			   struct exfat_parse_callback *pcb)
{
	...
	while (!ret && pos < dir->i_size) {
		...
		if (de_left) {
			...
			printk("%s: blocknr %llu, de_left %lu, offset %lu,"
			       " pos %llu, len %lu\n", __func__,
			...
			struct exfat_parse_callback *pcb)
{
	...
	if (data->type != EXFAT_TYPE_DATA)
		return PARSE_NEXT;
	printk("%s: wlen %u, name_len %u\n", __func__,
	       pcb_data->wlen, data->name_len);
	if (pcb_data->wlen != data->name_len)
		return PARSE_NEXT;
	printk("%s: hash 0x%04x, checksum 0x%04x\n", __func__,
			 ...
			 struct exfat_parse_callback *pcb)
{
	...
	str[0] = '\0';
	printk("%s: ent %p, str %s, len %u, pos %lld, ino %lu, type 0x%x\n",
...
static int exfat_readdir(struct file *filp, void *dirbuf, filldir_t filldir)
{
	...
	while (filp->f_pos < 2) {
		...
		printk("%s: ent %p, .., len %u, pos %lld, ino %lu, type 0x%x\n",
